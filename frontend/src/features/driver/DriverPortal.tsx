...
export const DriverPortal: React.FC = () => {
  ...
  const handleClaimJob = async (deliveryId: string) => {
    try {
      const coords = {
        latitude: driverProfile?.currentLatitude || 40.7180,
        longitude: driverProfile?.currentLongitude || -74.0010,
      };
  ...
  const driverLat = driverProfile?.currentLatitude || 40.7180;
  const driverLng = driverProfile?.currentLongitude || -74.0010;
  ...
  myJobs.forEach((j) => {
    ...
    if (don && recv) {
      ...
      mapRoutes.push({
        id: del.id,
        driverCoords: [driverLat, driverLng],
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                ...
                <div key={job.deliveryId} className="bg-white border border-stone-200 rounded-lg p-5 shadow-none space-y-4">
                  <div className="flex items-start justify-between">
                    ...
                    <div className="text-right">
                      <div className="text-sm font-mono font-bold text-amber-800">{job.totalEtaMinutes} min</div>
                  ...
                  <div className="bg-stone-50 p-3 rounded-lg border border-stone-200 space-y-2 text-xs">
                    <div className="flex items-center gap-2 text-stone-700">
                      ...
                      <div>
                        ...
                        <span className="text-xs text-stone-500 ml-2">~{job.distanceToPickupKm} km ({job.etaToPickupMinutes}m away)</span>
                      </div>
                    </div>
                    <div className="flex items-center gap-2 text-stone-700">
        ...
        <div className="space-y-4">
          ...
          {myJobs.length === 0 ? (
            <div className="bg-white border border-stone-200 rounded-lg p-8 text-center text-stone-600 text-xs">
              You have no active claimed deliveries. Claim an available job on the left to begin.
            </div>
          ) : (
            <div className="space-y-4">
              {myJobs.map((item) => {
